...
public class MediaDataAdapter extends RecyclerView.Adapter<MediaDataAdapter.VisualMediaAdapterViewHolder> {
    ...
    private ViewGroup.LayoutParams getPosterDimensions(View view) {
        ...
        if (mFragment.getResources().getBoolean(R.bool.isPhone)) {
            //get master fragment layout weight
            masterRatio = mFragment.getResources().getInteger(R.integer.search_main_layout_weight);
            detailRatio = mFragment.getResources().getInteger(R.integer.search_results_layout_weight);
    ...
    @NonNull
    @Override
    public MediaDataAdapter.VisualMediaAdapterViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
    ...
    public void onBindViewHolder(@NonNull VisualMediaAdapterViewHolder adapterViewHolder, int position) {
        ...
        MediaData currentVisualMediaData = mAdapterData.get(position);

        //if item view type is no poster
        if (adapterViewHolder.getItemViewType() == NO_POSTER_LAYOUT) {
            //set title instead of poster image
            adapterViewHolder.posterReplacementTextView.setText(currentVisualMediaData.getTitle());

        } else {
            //load image with glide
            NetworkFunctions.loadImage(
                    context,
                    currentVisualMediaData.getPosterPath(),
...
//    }
    ...
    public MediaData getVisualMediaData(int index) {
        return mAdapterData.get(index);
    }

    public List<MediaData> getVisualMediaDataList() {
        return mAdapterData;
    }
